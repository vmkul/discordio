...
module.exports = {
  ...
  execute(message, args) {
    const maps = ['Mirage', 'Inferno', 'Overpass', 'Vertigo', 'Nuke',
      'Train', 'Dust II', 'Anubis', 'Cache', 'Agency', 'Office'];
    const pool = [];

    const num = parseInt(args[0]);
    if (args.length === 0 || isNaN(num) || num > 11 || num < 1) {
      message.channel.send('Please specify a number of maps' +
        ' you want to include (from 1 to 11)');
      return;
    }
